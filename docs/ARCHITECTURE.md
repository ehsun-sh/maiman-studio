...
* **Transceivers.** Mode partition noise is computed at the laser by `dispersed_power`, not by a
  ...
  after a span does not see the delays between them; a laser's thermal wavelength drift with bias; bit-exact oFEC, which needs the OIF Implementation Agreement open rather than
  recalled.
