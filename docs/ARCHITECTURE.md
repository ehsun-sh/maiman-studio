...
* **OFEC, bit-exact.** The Open ROADM / G.709.6 code, from its formal definition, reproducing the
  specification's test vectors bit for bit through encoders and interleavers; decoded by this
  project's soft-in soft-out Chase iteration, since the specification leaves decoding open.
