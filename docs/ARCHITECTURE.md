...
* **OFEC, bit-exact, and the W-Port framing around it.** The Open ROADM / G.709.6 code, from its
  ...
  word, training sequences and pilot grid -- TP0 to TP7, against the same vectors.
...
* **Transceivers.** A band's arrival time is carried as one number per carrier, so a span's
  ...
  The W-Port's DPO path is not here: FlexO-6(e)/8(e) with probabilistic constellation shaping,
  whose shaping lookup table the published vectors do not exercise. Error marking is a flag per
  CRC32 rather than 802.3 error blocks, which needs a client layer this has none of. A laser's junction heats as one pole, so the
