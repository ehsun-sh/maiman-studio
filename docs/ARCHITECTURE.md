...
* **Short reach.** A PAM4 driver whose predistortion spaces a modulator's output powers evenly,
  and a symbol-spaced FFE/DFE trained on the reference and counted on its own decisions.
...
* **Transceivers.** A directly modulated laser and its chirp; a fractionally spaced or blind
  (decision-directed) PAM equaliser; bit-exact oFEC, which needs the OIF Implementation Agreement open rather than
  recalled.
* **Studio.** A spectral view of a block's scattering matrix — a grating, a ring, a coupler — rather
  than the numbers a run reports.
