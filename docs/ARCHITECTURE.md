...
* **OFEC, bit-exact, and the W-Port framing around it.** The Open ROADM / G.709.6 code, from its
  ...
  time for FlexO-8, which is what the specification quotes -- and checked against its TP3 and TP4.
...
* **Transceivers.** A band's arrival time is carried as one number per carrier, so a span's
  ...
  The DPO path stops at the encoder's input: that path's encoder output ordering is not the DO
  one, and clause 9.2.4 permutes the last 35 bits of each codeword back through the code, so TP4
  to TP5 is not modelled. The shaping tables themselves are normative data and are loaded from
  `MAIMAN_WPORT_TABLES` rather than shipped. Error marking is a flag per CRC32 rather than 802.3
  error blocks, which needs a client layer this has none of. A laser's junction heats as one pole, so the
