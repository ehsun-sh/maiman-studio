...
down. The MMI and the Mach-Zehnder followed as compositions of those, and PDK import after them. A
Y-junction is not a block of its own: a 1×2 MMI is the splitter a foundry ships.
...
### Phase 5 — Gratings, sensing, loops and coupling

None of this was in the original plan. Each piece arrived because something already shipped could not
be finished without it, and each is validated against a result it does not share code with.

* **Reflection.** A fibre Bragg grating assembled from per-section transfer matrices, which agree
  with Erdogan's closed form to twelve digits and then do what the closed form cannot: apodized,
  chirped, sampled and phase-shifted profiles are arrays handed to the same loop. A circulator
  routes the reflection, with isolation — which turned out not to be a loop, and needed
  `PortGroup` to allow inputs shared between groups — and return loss, which is one.
* **Sensing.** Strain and temperature move a grating by 1.209 pm/µε and 11.19 pm/K, read back by a
  swept laser or by broadband light on an analyser. The second needed noise that carries the
  spectral shape of what it passed through, rather than one flat density per bin.
...
* **A block's spectrum in the studio.** Any scattering device hands over its matrix across a
  window through `spectrum()`; the studio's S-matrix pane plots it as power, phase or group
  delay, the delay a local derivative rather than an unwrap.
* **OFEC, bit-exact, and the W-Port framing around it.** The Open ROADM / G.709.6 code, from its
  formal definition, reproducing the specification's test vectors bit for bit through encoders and
  interleavers; decoded by this project's soft-in soft-out Chase iteration, since the specification
  leaves decoding open. Either side of it, `maiman.wport` is the framing: the FlexO adaptation's
  CRC32s and pad, the frame-synchronous scrambler, the symbol mapper and the DSP frame's alignment
  word, training sequences and pilot grid -- TP0 to TP7, against the same vectors. On the DPO
  modes `maiman.pcs` adds clause 9's probabilistic constellation shaping in front of the code,
  transcoding the payload so the constellation's inner points come up more often -- 0.623 of the
  time for FlexO-8, which is what the specification quotes -- and clause 9.2.4's post-encode permute
  of each codeword's last 35 bits, applied to the encoder's matrix because the rows twenty behind
  read those bits as their fronts. `dpo_transmit` reproduces TP0 to TP7 for all three shaped
  modes.
* **Templates.** The studio's File menu opens complete projects, each generated by a script in
  `examples/`.
...
* **Transceivers.** A band's arrival time is carried as one number per carrier, so a span's
  ...
  The shaped DPO path transmits to TP7 but cannot yet be decoded: clause 9.2.4's permute is
  applied to the encoder's matrix, so a codeword's front half is read as received while its back
  half has to be untangled, and holding both views is a change inside the component decoder. The
  shaping tables themselves are normative data and are loaded from `MAIMAN_WPORT_TABLES` rather
  than shipped. Error marking is a flag per CRC32 rather than 802.3 error blocks, which needs a
  client layer this has none of. A laser's junction heats as one pole, so the
