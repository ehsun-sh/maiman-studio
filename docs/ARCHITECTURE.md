...
* **OFEC, bit-exact, and the W-Port framing around it.** The Open ROADM / G.709.6 code, from its
  ...
  modes, and `dpo_receive` runs it backwards: the decoder holds two views of one matrix, fronts as
  the line carried them and backs untangled, which is the only reading under which a clean stream
