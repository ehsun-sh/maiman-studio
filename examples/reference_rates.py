...
def main() -> None:
    ...
    print(
        ...
        "  polarization on this bench, so the blind butterfly equaliser has nothing\n"
        "  to undo and should cost nothing — and at 16-QAM it does not. At 64-QAM it\n"
        "  diverges: nine constellation radii sit close enough together that a noisy\n"
        "  sample snaps to the wrong one, and the correction that follows is large\n"
        "  and in the wrong direction. A smaller step recovers most of it and a\n"
        "  single tap all of it, so the structure is right and the adaptation is not.\n"
        "  A normalised update is the textbook fix; it moves every other format's\n"
        "  answer too, so it is a separate piece of work and not this one."
    )
